  ...
  <div class="article-container">
      ...
      <template v-else>
        ...
        <div class="article-detail" v-if="!!article.art_id">
          ...
          <ArticleComment
            ...
            "
          ></ArticleComment>
          <!-- commentList 把这个传到ArticleComment中去 -->
        </div>
        <!-- /加载完成-文章详情 -->
    ...
    <div class="article-bottom" v-if="!isLoading && !!article.art_id">
      <van-button
        ...
        >写评论</van-button
      >
      <van-icon name="comment-o" color="#777" :badge="count" />
      <!-- <van-icon color="#777" name="star-o" /> -->
      <!-- 这是收藏五角星组件 将article.is_collected传给组件CollectArticle-->
      <CollectArticle
      ...
      <van-popup v-model="addCommentShow" position="bottom">
        <!-- 每次关闭 组件销毁  每次打开 组件重新创建  mounted就会再一次执行来获得焦点 -->
        <!-- 将文章id值传到AddComment里面 -->
        <!-- 把发布成功得最新评论放到评论列表最前面  关闭弹出层  接受addComment传过来的 -->
        <AddComment
          v-if="addCommentShow"
          :target="article_id"
          @add-comment="
...
// 因为只有这一块需要 所以github-markdown-css不用全局引入
...
export default {
  ...
  async created () {
    try {
      ...
      // 如果两个同时成立 this.is404Error = true
      if (err.response && err.response.status === 404) {
        this.is404Error = true
      }
    }
    this.isLoading = false
    // 在这块的时候loading关闭了，页面就显示正文，但是必须还是得写在nextTick中
    // 因为数据更新，数据不能立即更新  写在nextTick回调中可以，因为nextTick回调
    // 就是在视图更新完成才会触发
    this.$nextTick(() => {
